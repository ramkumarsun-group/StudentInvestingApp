...
export default function ClassDetailPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <Link href="/teacher/classes" className="flex items-center gap-1 text-on-surface-variant hover:text-on-surface text-sm">
        <ChevronLeft size={16} />
        My Classes
      </Link>
        ...
        <>
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-on-surface-variant mt-1">{cls.semester}</p>
            </div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {cls.students.map((s, i) => (
                    <tr key={s.id} className="border-b border-surface-container-high hover:bg-surface-container-high/30">
                      <td className="px-5 py-3 text-on-surface-variant text-sm">#{i + 1}</td>
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          <div className="w-7 h-7 bg-primary-container rounded-full flex items-center justify-center text-xs font-bold shrink-0">
                            {s.username[0].toUpperCase()}
                          </div>
                          <div>
                            <p className="text-sm text-white font-medium">{s.username}</p>
                            <p className="text-xs text-on-surface-variant">{s.level_name}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-3 text-right text-sm text-on-surface-variant font-mono">{formatUSD(s.total_value ?? 100000)}</td>
                      ...
                      <td className="px-5 py-3 text-right text-sm text-on-surface-variant">{s.lessons_completed ?? 0}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
