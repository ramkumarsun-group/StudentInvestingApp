    ...
    <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="card p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm text-on-surface-variant mb-1">Class Name</label>
                <input className="input" placeholder="AP Finance Period 1" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
              </div>
              <div>
                <label className="block text-sm text-on-surface-variant mb-1">Semester</label>
                <input className="input" placeholder="Fall 2025" value={form.semester} onChange={(e) => setForm({ ...form, semester: e.target.value })} />
              </div>
              <div>
                <label className="block text-sm text-on-surface-variant mb-1">Academic Year</label>
      ...
      {classes.length === 0 ? (
        <div className="card p-12 text-center">
          <GraduationCap size={48} className="text-on-surface-variant mx-auto mb-4" />
          <p className="text-on-surface-variant">No classes yet. Create your first class to get started.</p>
        </div>
      ) : (
          ...
          {classes.map((c) => (
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-semibold text-white group-hover:text-primary transition-colors">{c.name}</h3>
                  <p className="text-on-surface-variant text-sm mt-0.5">{c.semester} · {c.academic_year}</p>
                </div>
              ...
              <div className="flex items-center gap-4 mt-4 text-sm">
                ...
                <span className="bg-surface-container-high text-on-surface-variant px-2 py-0.5 rounded font-mono text-xs tracking-widest">
                  {c.join_code}
                </span>
              </div>
            </Link>
